...
const ViewSearch = ({ flagSearch }) => {
  ...
  const dataclima = useAxios('https://api.weatherapi.com/v1/current.json?key=6be8c28794924ed8a2a184922222905&q=', location);
  return (
    <>
      <button className="btn btn-primary" onClick={flagSearch}>
        Search City
      </button>
      {!dataclima.loading && (
        <Card
          temp={dataclima.data.current.temp_c}
